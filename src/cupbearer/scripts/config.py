...
ONLINE_SCORE_ORDER: List[str] = [s for s in SCORE_ORDER if not any(j in s for j in ["likelihood-ratio", "que", "expectation-maximization"])]
OFFLINE_SCORE_ORDER: List[str] = [s for s in SCORE_ORDER if any(j in s for j in ["likelihood-ratio", "que", "expectation-maximization"])]
