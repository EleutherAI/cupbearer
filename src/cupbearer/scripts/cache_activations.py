...
datasets = [
    ...
    "authors", "addition", "subtraction", "multiplication", "modularaddition", "squaring"
]

def build_cache(dataset, layers, batch_size):
    print(f"Processing dataset: {dataset}")

    if dataset in ['sciq', 'sentiment']:
        batch_size //= 4

    task = tasks.quirky_lm(include_untrusted=True, mixture=True, standardize_template=True, dataset=dataset, easy_quantile=1, hard_quantile=0)
...
def main(first_layer, last_layer, batch_size):
    ...
    for dataset in datasets:
        build_cache(dataset, layer_list, batch_size)
